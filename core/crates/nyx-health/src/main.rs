...
// `wheel` always exists on Arch and is exactly the trust boundary we want:
// anyone who can already `sudo` to root can talk to the daemon directly too.
const SOCKET_GROUP: &str = "wheel";

fn bind_socket() -> std::io::Result<UnixListener> {
    std::fs::create_dir_all(RUNTIME_DIR)?;

    let path = Path::new(HEALTH_SOCKET);
    if path.exists() {
...
async fn main() -> anyhow::Result<()> {
    ...
    if !nix::unistd::Uid::effective().is_root() {
        anyhow::bail!("nyx-health must run as root (it owns firewall + service control)");
    }
    ...
    tracing::info!("nyx-health listening on {HEALTH_SOCKET}");
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                let response = match serde_json::from_str(&line) {
                    ...
                    Err(e) => nyx_core::NyxOutput::err("nyx-health", "parse", e.to_string()),
                };
