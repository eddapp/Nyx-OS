...
use crate::{
    amneziawg, cloak, dante, free_provider, hysteria, import, mieru, openvpn, route, shadowsocks,
    templates, wireguard, xray,
};
...
fn list_profiles() -> Vec<VpnProfile> {
    ...
    // No provider/import writer targets mieru yet, so nothing can have
    // written it an INCOMPLETE_MARKER — `false` is honest, not a stub.
    profiles.extend(
        mieru::list_profiles()
            .into_iter()
            .map(|name| VpnProfile { protocol: VpnProtocol::Mieru, name, incomplete: false }),
    );
    profiles
}
