...
// Same trust boundary as nyx-health: anyone who can already `sudo` on this
// system can read DNS state without an extra prompt on every poll.
const SOCKET_GROUP: &str = "wheel";

fn bind_socket() -> std::io::Result<UnixListener> {
    std::fs::create_dir_all(RUNTIME_DIR)?;

    let path = Path::new(DNS_SOCKET);
    if path.exists() {
...
async fn main() -> anyhow::Result<()> {
    ...
    if !nix::unistd::Uid::effective().is_root() {
        anyhow::bail!("nyx-dns must run as root (it inspects other processes' sockets)");
    }
    ...
    tracing::info!("nyx-dns listening on {DNS_SOCKET}");
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                let response = match serde_json::from_str(&line) {
                    ...
                    Err(e) => nyx_core::NyxOutput::err("nyx-dns", "parse", e.to_string()),
                };
