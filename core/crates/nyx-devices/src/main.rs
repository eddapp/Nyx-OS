...
// `sudo` on this system can flip these switches without an extra prompt.
const SOCKET_GROUP: &str = "wheel";

fn bind_socket() -> std::io::Result<UnixListener> {
    std::fs::create_dir_all(RUNTIME_DIR)?;

    let path = Path::new(DEVICES_SOCKET);
    if path.exists() {
...
async fn main() -> anyhow::Result<()> {
    ...
    if !nix::unistd::Uid::effective().is_root() {
        anyhow::bail!(
            "nyx-devices must run as root (it drives nmcli/rfkill/modprobe/wpctl and USBGuard)"
        );
    }
    ...
    tracing::info!("nyx-devices listening on {DEVICES_SOCKET}");
        ...
        tokio::spawn(async move {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                let response = match serde_json::from_str(&line) {
                    Ok(cmd) => handler::dispatch(&conn, cmd).await,
                    Err(e) => nyx_core::NyxOutput::err("nyx-devices", "parse", e.to_string()),
                };
